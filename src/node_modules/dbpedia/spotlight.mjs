...
export const annotateIndex = async (
	...
	const mappingPayload = generateMappingPayload(newFieldName, includeMetaData);
	await updateMapping(domain, index, { payload: mappingPayload });

	// const bar = await initialiseIndexProgressBar(domain, index, batchSize);
	...
	for await (page of scroller) {
		...
		for (const docs of batches) {
			...
			const annotatedBatch = await annotateBatch(
				docs, fieldName, newFieldName, endpoint, includeMetaData
			);
		...
		bulkRequest(domain, index, flattenedUpdates, 'update', { error: false });
	}
