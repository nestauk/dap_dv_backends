	...
	const aggs = confidences.reduce((acc, conf) => {
		return {
			...
			[`entities_count_over_token_count_at_${conf}_extended_stats`]: {
				extended_stats: {
					script: `if (doc['dbpedia_entities_metadata.confidence_counts.${conf}'].size() == 0 || doc['${textField}.token_count'].size() == 0) { return 0; } return ((double) doc['dbpedia_entities_metadata.confidence_counts.${conf}'].value) / doc['${textField}.token_count'].value;`
				}
			},
